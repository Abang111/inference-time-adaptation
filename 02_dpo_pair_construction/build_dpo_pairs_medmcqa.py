...
"""
edprm_policy_sampling_rerank_dataset_v5_medmcqa.py
...
# ---------- Input FULL docs ----------
FULL_DOCS_ROOT = Path("retrieval_full_docs_run").resolve()
DOCS_DIR = FULL_DOCS_ROOT / "docs"

# FULL train docs only, medmcqa ONLY
INPUT_FILES = [
...
POLICY_FT_8B_DEVICE = "cuda:4"
POLICY_FT_8B_DEVICE_MAP = {"": POLICY_FT_8B_DEVICE}

POLICY_8B_DEVICE = "cuda:5"
...
def score_batch_plus_minus(
    ...
    """Score multiple prompts in a single batched forward pass."""
    if not prompt_texts:
        return []
    if len(prompt_texts) == 1:
        return [score_next_token_plus_minus(model, tokenizer, prompt_texts[0], plus_ids, minus_ids)]
    ...
    for idx in range(len(prompt_texts)):
        ...
        last_pos = int(seq_mask.sum().item()) - 1
        ...
        results.append({
            "p_plus": float(probs[0].item()),
            "p_minus": float(probs[1].item()),
            "margin": float(probs[0].item()) - float(probs[1].item()),
        })
...
def build_policy_prompt(policy_tokenizer, question: str, docs: List[str]) -> str:
    if POLICY_INCLUDE_DOCS:
        ...
        system = (
            ...
            "Step 1: <one medically meaningful reasoning sentence>\n"
            "Step 2: <one medically meaningful reasoning sentence>\n"
            "Step 3: <one medically meaningful reasoning sentence>\n"
            "Final Answer: (X)\n\n"
            "STRICT RULES:\n"
            "- You MUST use exactly 3 to 6 steps. No more, no less.\n"
            "- You MUST end with 'Final Answer: (X)' where X is A, B, C, D, or E.\n"
            "- NEVER omit the Final Answer line. It is MANDATORY.\n"
            "- Each step must be a single concrete medical claim or inference, not a vague summary.\n"
            "- Do NOT write bullet points, markdown, sub-steps, or extra headings.\n"
            "- Do NOT write anything after the Final Answer line."
        )
        user = (
            f"Documents:\n{doc_block}\n\n"
            f"Question:\n{question.strip()}\n\n"
            "Respond using ONLY the Step 1/2/3/.../Final Answer format. "
            "Do NOT forget the Final Answer line."
        )
    else:
        system = (
            "You are a medical reasoning assistant.\n"
...
def score_trace_steps(
    ...
    # Build all step prompts, then score in a single batch
    prompts = []
    ...
    batch_results = score_batch_plus_minus(
        model=rm_model,
        tokenizer=rm_tokenizer,
    ...
    step_results = []
    for i, (step, out) in enumerate(zip(steps, batch_results), start=1):
        step_results.append({
            "step_index": i,
            "step_text": step,
            **out,
...
def main():
    ...
    print("[INFO] GPU layout: RM->cuda:3, PolicyFT->cuda:1, Policy8B->cuda:2")
    print("[INFO] Dataset: medmcqa ONLY")
    ...
    for item in questions:
        ...
        for oom_retry in range(MAX_OOM_RETRIES + 1):
            try:
                if oom_retry > 0:
                    print(f"[RETRY] qid={qid} | OOM retry {oom_retry}/{MAX_OOM_RETRIES}")
                ...
                print(f"[OOM] qid={qid} | retry {oom_retry}/{MAX_OOM_RETRIES}")
                gc.collect()
                torch.cuda.empty_cache()
                if oom_retry >= MAX_OOM_RETRIES:
                    print(f"[SKIP-OOM] qid={qid} | exhausted {MAX_OOM_RETRIES} retries")
                    append_jsonl(SKIP_JSONL, {
                        "qid": qid,
                        "dataset": item.get("dataset", ""),
                        "reason": "oom_exhausted",
                        "error": f"OOM after {MAX_OOM_RETRIES} retries",
                    })
